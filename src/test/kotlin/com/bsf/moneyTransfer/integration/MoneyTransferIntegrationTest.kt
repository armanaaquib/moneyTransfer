...
internal class MoneyTransferIntegrationTest(@Autowired private val restTemplate: TestRestTemplate) {
    ...
    fun `should transfer money`() {
        ...
        val entity = restTemplate.postForEntity("/moneyTransfers", moneyTransferRequest, ApiResponse::class.java)
    ...
    fun `should get error when receiver account id not valid`() {
        ...
        val expectedResponse =
            ApiResponse<Any>(error = ApiError("Could not find a account with ${moneyTransferRequest.receiverAccountNumber} account number"))
