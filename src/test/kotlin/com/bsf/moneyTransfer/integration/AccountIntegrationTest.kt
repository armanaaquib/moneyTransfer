...
internal class AccountIntegrationTest(@Autowired private val restTemplate: TestRestTemplate) {
    ...
    fun `should create account`() {
        ...
        val expectedResponse = SuccessResponse(Account(accountNumber, Money(BigDecimal(0))))
    ...
    fun `should get account when account is present`() {
        ...
        val expectedResponse = SuccessResponse(Account(accountNumber, Money(BigDecimal(100))))
    ...
    fun `should get error when account is not present`() {
        ...
        val expectedResponse =
            FailureResponse(error = ApiError("Could not find an account with $accountNumber account number"))
